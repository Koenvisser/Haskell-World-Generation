...
data PerlinConfig = PerlinConfig {
  seed :: Int,          -- ^ The seed used to generate the permutations. Can also be generated randomly using the `perlinNoiseRandom` function.
  permSize :: Int,      -- ^ The size of the permutation table. Must be greater than 0.
  octaves :: Int,       -- ^ The number of octaves used to generate the noise. Must be greater than or equal to 0. 
...
permutationGenerator size rng | size <= 0 = error "Permutation size must be greater than 0"
                              | otherwise = M.fromList $ zip [0 .. size - 1] $ shuffle' [0 .. size - 1] size rng
        
-- | Generate a heightmap using Perlin noise.
perlinNoise :: PerlinConfig -> HeightMap
perlinNoise pcf = let perm = permutationGenerator (permSize pcf) (mkStdGen $ seed pcf)
...
noiseGenerator config perms (x, y) | octaves config < 0 = error "Octaves must be greater than or equal to 0"
                                   | octaves config == 0 = 0
...
noiseGenerator' config perms amp (x, y) | octaves config < 0 = error "Octaves must be greater than or equal to 0"
                                        | octaves config == 0 = (0, 0)
                                        | otherwise = let
